...
class Students extends Members {
    ...
    static isStudentIdValid(studentID) {
        return Students.studentInfo.find(id => id.sId === studentID);
    }
    #student = {
        sId : 0,
        sGrade : '',
    ...
    constructor(studentName, studentAge, studentCountry, studentCourse) {
        ...
        if (studentAge >= 16 && this.#isCourseNameValid(studentCourse) && studentCountry.toLowerCase().localeCompare('usa') === 0 ) {
            ...
            Students.studentInfo.push(this.#student)
    ...
    */
    ...
    changeCourse(newCourseName) {
        if (this.#isCourseNameValid(newCourseName)) {
            ...
            this.#student.sCourseDetails.courseLength = courseData.length;
            this.#student.sBalance = courseData.price;
        } console.log( `Updated course name: ${this.#student.sCourseDetails.courseName}\n New Balance: $${this.#student.sBalance}\n`);
    }

    payBalance(amountPayed) {
        if (amountPayed > 0 && amountPayed <= this.#student.sBalance) {
            this.#student.sBalance = this.#student.sBalance - amountPayed
            console.log(`Thanks for your payment, your new balance is $${this.#student.sBalance}`);
        } else {
            console.log(`Your payment must be at least 1$ and no larger than your current balance of $${this.#student.sBalance}`);
        }
    }
}
